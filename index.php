...
$(document).ready(function() {
    // Target the correct table: #shippedDataTable
    console.log("Attempting to disable pagination for #shippedDataTable");
    
    // Try multiple approaches
    
    // Approach 1: Direct configuration
    if ($.fn.DataTable && $.fn.DataTable.isDataTable('#shippedDataTable')) {
        console.log("Found #shippedDataTable - reconfiguring");
        $('#shippedDataTable').DataTable().destroy();
        $('#shippedDataTable').DataTable({
            "paging": false,
            "info": false
        });
    }
    
    // Approach 2: Force page length to show all records
    $('.dataTables_length select').val('-1').change();
    
    // Approach 3: CSS to hide pagination
    ...
    // Approach 4: Remove pagination DOM elements
    setTimeout(function() {
...
<body>
            ...
            $('.list-group-item').click(function() {
                ...
                switch(targetSection) {
                    ...
                    case 'reports':
                        ...
                        break;
                    case 'csv-history':
                        loadCsvHistoryData(); // CHANGED: Call a new function
                        break;
    ...
    fetch('check_mismatches.php')
        ...
        .then(data => {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        var shippedDataTable = $('#shippedDataTable').DataTable({
                            ...
                            // ADD THESE LINES TO DISABLE PAGINATION:
                            "paging": false,
                            "info": false, // Also hides "Showing x to y of z entries"
                            // You can keep other options like ordering, searching if you have them
                            // "ordering": true,
                            // "searching": true,
            ...
            function loadCsvHistoryData() {
                console.log("Loading CSV History Data from API...");
                
                // Destroy existing DataTable instance if it exists, to prevent errors and ensure fresh load
                if ($.fn.DataTable.isDataTable('#csvHistoryTable')) {
                    $('#csvHistoryTable').DataTable().destroy();
                    $('#csvHistoryTable tbody').empty(); // Clear existing rows
                }

                $('#csvHistoryTable').DataTable({
                    "ajax": {
                        "url": "api_csv_history.php", // Corrected URL
                        "type": "GET",
                        "dataType": "json",
                        "dataSrc": "" // Important: DataTables expects an array, not an object with a 'data' property
                    },
                    "columns": [
                        { "data": "upload_date", "title": "Upload Date" },
                        { "data": "file_name", "title": "File Name" },
                        { "data": "records_added", "title": "Records" } // Changed title for brevity
                        // Temporarily remove columns with complex render functions
                        // { "data": "order_ids", "title": "Order IDs", /* render: function... */ },
                        // { "data": "filepath", "title": "Actions", /* render: function... */ }
                    ],
                    "processing": true,
                    "destroy": true,
                    "error": function (xhr, error, thrown) { // Add a DataTables specific error handler
                        console.error("DataTables AJAX error:", error, thrown);
                        console.error("XHR Response:", xhr.responseText);
                        alert("Error loading data into table. See console for details.");
                        $('#csvHistoryTable tbody').html('<tr><td colspan="3" class="text-center text-danger">Error loading history data. DataTable AJAX failed.</td></tr>');
                    }
                });
            }

            // Event listener for the "View Order IDs" button (delegated)
            // Ensure this is placed where it runs once, e.g., in your main $(document).ready()
            $(document).on('click', '.view-order-ids', function() {
                var ids = $(this).data('ids');
                if (window.showOrderIds && typeof window.showOrderIds === 'function') {
                    window.showOrderIds(ids);
                } else {
                    // Fallback or define showOrderIds here if it's not global
                    // For example, if the modal and its JS are loaded with index.php
                    $('#orderIdsList').html(String(ids).split(',').join('<br>'));
            ...
            // Updated handler that correctly interprets "affected: 0" as success
$(document).on('click', '#saveDateChanges', function(e) {
    ...
    $('#shippedDataTable .row-checkbox:checked').each(function() {
        ...
        if (id) {
            ...
            dateInput.css('background-color', '#fff3cd'); // Yellow background during processing
        }
    });
    
    if (selectedRecords.length === 0) {
        alert('Please select at least one row');
        $(this).prop('disabled', false).text('Save Date Changes');
        return;
    }
    
    $.ajax({
        url: 'update_dates.php',
        type: 'POST',
        dataType: 'json',
        data: {
        ...
        success: function(response) {
            // Consider both cases as success:
            // 1. success=true with affected > 0 (data changed)
            // 2. success=true with affected = 0 (no changes needed)
            if (response.success) {
                var message = response.updated > 0 ? 
                    response.updated + ' records updated' : 
                    'Dates were already up to date';
                ...
                $('#shippedDataTable .row-checkbox:checked').each(function() {
                    ...
                    row.find('.planned-delivery-input')
                        .css('background-color', '#d4edda') // Green background
        ...
        complete: function() {
            // Re-enable button
            $('#saveDateChanges').prop('disabled', false).text('Save Date Changes');
        }
    });
});
        });
